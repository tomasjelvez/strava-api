...
export async function GET(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("Strava route TCX export error:", res.status, text);
      if (res.status === 404) {
        return NextResponse.json(
          {
            error:
              "Strava returned 404 for TCX export. Private routes need read_all—reconnect Strava in Settings—or the route id may be wrong.",
    ...
    return new NextResponse(body, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="strava-route-${encodeURIComponent(id)}.tcx"`,
      },
    });
  } catch (err) {
    ...
    console.error("Strava route TCX export failed:", err);
    return NextResponse.json(
