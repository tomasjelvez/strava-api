...
- **`DATABASE_URL`** — connection string used at **runtime** by the app. With Supabase or another PgBouncer pooler, use the **pooled** Prisma-style URL here. The app uses [`@prisma/adapter-pg`](https://www.prisma.io/docs/orm/prisma-client/setup-and-configuration/databases-connections#external-connection-poolers) with this value. For **Supabase** (hosts matching **`*.pooler.supabase.co`**, **`db.*.supabase.co`**, or **`new URL` hostname `*.supabase.co`**), the pool uses **`ssl: { rejectUnauthorized: false }`** and **strips TLS-related query params** (`sslmode`, `uselibpqcompat`, etc.) from the URL so **`pg`** does not treat `sslmode=require` like strict verify-full (**P1011** / *self-signed certificate in certificate chain* on Vercel). Outside that relaxed path, **`uselibpqcompat=true`** may still be appended when the URL parses. Set **`DATABASE_SSL_VERIFY=1`** to force strict TLS for Supabase. For non-Supabase databases, set **`DATABASE_SSL_REJECT_UNAUTHORIZED=0`** if you need the same relaxed behavior. If the DB password contains **`@`, `:`, `/`, `#`**, percent-encode them in the URI so tooling can parse the URL; detection still matches **`pooler.supabase.co`** / **`db.*.supabase.co`** substrings even when `new URL` fails.
